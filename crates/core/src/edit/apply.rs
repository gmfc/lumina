...
pub fn edit_selections<F>(doc: &mut Document, f: F, group: GroupBreak)
where
    F: FnMut(&Document, Selection) -> (Range<usize>, String),
{
    let before = doc.selections.clone();
    // Build the transaction + resulting selections purely (no mutation), then apply here. Sharing
    // the build with the Host-facing path keeps a single source of truth for how a multi-selection
    // op becomes a `Transaction` — the caret recompute reads only the ops, never the mutated doc,
    // so computing it before the apply is identical to computing it after.
    let (txn, after) = selection_edit_transaction(doc, f);
    if txn.is_empty() {
        return;
    }
